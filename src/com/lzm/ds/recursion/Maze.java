...
public class Maze {
     ...
     */
    /**
     * 老师的找迷宫方法: 如果终点坐标被走过了，直接结束函数。如果该坐标的数值为0，假设该坐标可以走，先给他赋值为2，然后进行下右上左地走。如果上下左右都走不通，赋值该点为3（死路）
     * 如果是该点的坐标不是0，有可能是1，也有可能是3，返回false
     */
    private static boolean teacher_SeekWay(int[][] maze,int row,int col) {
        if (maze[6][5] == 2)
            return true;
        if (maze[row][col] == 0) {
            // 假设这个点可以走得通，给他赋值为2，假设下右上左都走不了，就赋值3（死路）
            maze[row][col] = 2;
            // 进行下右上左地走
